# Delta for Plant Registration Flow
...
(Previously: No `posicion_en_fila` field; no progress indicator; no `num_plantas_esperadas` awareness.)
...
(Previously: No change — this requirement remains unchanged.)
...
(Previously: Batch creation did not include `posicion_en_fila`.)
...
(Previously: No change — role-based access remains unchanged.)
