...
Define the frontend UI and backend restore endpoint for managing irrigation events — creating, listing, editing, soft-deleting, and restoring events that track when and how much water was applied to each plot.

## Requirements

### Requirement: View Irrigation Events

The system SHALL display a paginated or scrollable list of all active irrigation events across all plots, ordered by fecha DESC. Each event card SHALL show: plot name (from JOIN), fecha, sistema_tipo (from JOIN on irrigation_systems), duracion_min, mm_aplicados, and observaciones. The system SHALL show a loading state while fetching and an empty state when no events exist.
...
- AND displays each event as a card showing plot name, fecha, sistema_tipo, duracion_min, mm_aplicados, observaciones
...
The system SHALL provide an inline form to create a new irrigation event. The form SHALL include: plot selector (dropdown of all active plots), irrigation system selector (dropdown of all active systems), fecha (date input, default today), duracion_min (number, optional), mm_aplicados (number, optional), observaciones (textarea, optional). The fields plot, irrigation system, and fecha SHALL be required. On success, the event SHALL appear in the list and the form SHALL clear. On error, the system SHALL display an alert with the error message.

#### Scenario: Create event with all fields

- GIVEN the user is on `/irrigation-events` with the create form visible
- WHEN the user selects a plot, selects a system, enters fecha, duracion_min, mm_aplicados, and observaciones
...
- WHEN the user selects a plot, selects a system, enters fecha, and submits (leaving optional fields blank)
- THEN the system creates the event with null duracion_min, mm_aplicados, observaciones
- AND the event appears in the list

#### Scenario: Validation — missing required field

- GIVEN the create form is visible
- WHEN the user submits without selecting a plot, system, or fecha
